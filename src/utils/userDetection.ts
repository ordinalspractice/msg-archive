...
const AutofillDataSchema = z.object({
...
const AutofillInfoSchema = z.object({
...
export const detectUserName = async (
  ...
  try {
    ...
    logger.debug('USER_NAME_DETECTED', { 
      userName, 
      source: fullName ? 'FULL_NAME' : (firstName && lastName) ? 'FIRST_LAST' : firstName ? 'FIRST_ONLY' : 'NONE'
    });
    return userName;
    
  } catch (error) {
    logger.debug('AUTOFILL_READ_FAILED', { error: error instanceof Error ? error.message : 'Unknown' });
    return null;
  }
};